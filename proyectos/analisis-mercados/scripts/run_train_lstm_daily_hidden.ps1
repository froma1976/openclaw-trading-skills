...
try {
  ...
  # 4) Resumen diario aprendizaje
  $out3 = & $py -3 "C:\Users\Fernando\.openclaw\workspace\proyectos\analisis-mercados\scripts\learning_daily.py" 2>&1
  $exit3 = $LASTEXITCODE
  if ($out3) { $out3 | Add-Content $log }
  "[$(Get-Date -Format s)] LEARNING_EXITCODE=$exit3" | Add-Content $log

  if ($exit0 -ne 0 -or $exit1 -ne 0 -or $exit2 -ne 0 -or $exit3 -ne 0) {
    exit 1
  }
  exit 0
} catch {
